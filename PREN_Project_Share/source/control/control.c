...
bool Ramp_Disabled = true; //ramp disabled
...
{
	Ramp_Disabled = command.Disabled;//rampe
	if(Coil_before){
	...
	if (((command.Steps1 != 0)||(command.Steps2 != 0)||(command.Steps3 != 0)||(command.StepsRot != 0))&&(command.ErrorHandling==false)){
		moveWay(command.Steps1,command.Steps2,command.Steps3, command.StepsRot, Ramp_Disabled);

	}else if (command.ErrorHandling==true){
		// Use last saved Values but with contrary direction
		moveWay(M1_Last_Step,M2_Last_Step,M3_Last_Step,0,0);
#if ENABLE_ROT
